...
- The service also exposes deterministic corpus interaction networks with speaker, edge, centrality, and connectivity metrics.
- Reply-aware, baseline-corrected lexical coordination is available through
  `reply-coordination`, with speaker metadata identities, support thresholds,
  and explicit counts; see [the coordination guide](docs/reply-coordination.md).
- The legacy `coordination` command and service operation retain their adjacent-role
  conditional rates without baseline correction.
